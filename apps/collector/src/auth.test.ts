...
test('API is gated when ARA_TOKEN is set; /health stays open', async () => {
  ...
  try {
    ...
    // De handelspaden zijn het gevoeligst van allemaal: stand, voorstellen en
    // de noodstop mogen nooit zonder token bereikbaar zijn.
    for (const route of ['/trade/state', '/trade/intents', '/trade/halt', '/Trade/state']) {
      assert.equal((await fetch(`${base}${route}`)).status, 401, `${route} gated`);
    }
